...
# Custom Simple GBM implementation
...
# Step 1: Load Litecoin dataset
df = pd.read_csv('Dataset/coin_Litecoin.csv', parse_dates=['Date'])
...
# Step 3: Log Transformation on Target
...
# Step 6: Hyperparameter tuning with K-Fold using SimpleGBMRegressor
param_grid = {
    'n_estimators': [100],
    'learning_rate': [0.1],
    'max_depth': [5]
...
joblib.dump(final_model, 'models/best_xgboost_litecoin_model.pkl')
print("\nModel saved to models/best_xgboost_litecoin_model.pkl")

# Step 14: Predict future prices
...
# Step 16: Plot future predictions
plt.scatter(simulated_dates, simulated_prices, color='green', label='Future Predictions', marker='o')

plt.title('Litecoin Price Prediction: Actual, Predicted (Train), and Future Prices')
plt.xlabel('Date')
